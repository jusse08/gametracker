def test_superadmin_can_delete_regular_user(client, auth_headers):
...
def test_cannot_delete_superadmin(client, auth_headers):
    users_response = client.get("/api/users", headers=auth_headers)
    assert users_response.status_code == 200, users_response.text
    ...
    delete_response = client.delete(f"/api/users/{superadmin['id']}", headers=auth_headers)
